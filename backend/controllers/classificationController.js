...
// @desc    Get all classifications history for current user (or global archive)
// @route   GET /api/classification
// @access  Protected
const getClassifications = async (req, res) => {
  try {
    // Build user filter condition: if user is Admin or for global history, fetch all records.
    // Otherwise, fetch records created by user OR unassigned/legacy records.
    const userQuery =
      req.user?.role === "Admin"
        ? {}
        : {
    ...
    // Retrieve material classifications populated with uploaded image metadata
    let materialClassifications = await MaterialClassification.find(userQuery)
      .populate("uploadedImage")
      .sort({ createdAt: -1 });

    // Fallback: If no records found for strict query, load all material classifications for complete past profile archive
    if (!materialClassifications || materialClassifications.length === 0) {
      materialClassifications = await MaterialClassification.find({})
        .populate("uploadedImage")
        .sort({ createdAt: -1 });
    }

    // Retrieve waste classifications
    const wasteClassifications = await WasteClassification.find({}).lean();

    // Retrieve sustainability records
    const sustainabilityRecords = await SustainabilityRecord.find({}).lean();
    ...
    let standaloneAnalyses = await Analysis.find(userQuery).sort({ createdAt: -1 });
    if (!standaloneAnalyses || standaloneAnalyses.length === 0) {
      standaloneAnalyses = await Analysis.find({}).sort({ createdAt: -1 });
    }
    ...
    const analysisHistory = await Promise.all(
      standaloneAnalyses.map(async (a) => {
        ...
        try {
          ...
          console.warn("Sustainability calculation fallback warning for standalone analysis:", err.message);
        }
...
const deleteHistoryRecord = async (req, res) => {
  try {
    ...
    const targetCreator = mc?.createdBy || analysis?.createdBy;
    if (targetCreator && targetCreator.toString() !== req.user._id.toString()) {
      return res.status(403).json({
        success: false,
        message: "Not authorized to delete this analysis record.",
      });
    }
